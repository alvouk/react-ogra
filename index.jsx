...
function uniqueOptions(correct, pool, count = 3) {
  const set = new Set();
  set.add(correct);

  const others = pool.filter((p) => p !== correct);
  const shuffled = shuffleArray(others);
  for (let i = 0; i < shuffled.length && set.size < count; i++) {
    set.add(shuffled[i]);
  }
...
export default function JakieToUbranieQuiz({ questionsCount = 10 }) {
  ...
  useEffect(() => {
    const shuffled = shuffleArray(IMAGE_POOL);
    setSelectedImages(shuffled.slice(0, Math.min(questionsCount, shuffled.length)));
    setCurrentIndex(0);
    setScore(0);
  }, [questionsCount, seed]);
  ...
  const current = selectedImages[currentIndex] ?? null;

  const kindOptions = useMemo(() => {
    if (!current) return [];
    return uniqueOptions(current.kind, CLOTHING_KINDS, 3);
  }, [current]);

  const colorOptions = useMemo(() => {
    if (!current) return [];
    return uniqueOptions(current.color, COLORS, 3);
  }, [current]);
  ...
  function nextQuestion() {
    ...
    setCurrentIndex(selectedImages.length);
  }
      ...
      <main className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        <section className="bg-white rounded-lg p-4 shadow">
          ...
          <div className="mt-4 flex gap-2">
            ...
            {answered && currentIndex + 1 === selectedImages.length && (
              <button onClick={nextQuestion} className="px-4 py-2 rounded border">Zobacz wynik</button>
            )}
